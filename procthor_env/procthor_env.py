...
class ThorEnv(Controller):
    '''
    an extension of ai2thor.controller.Controller for ProcTHOR tasks
    '''
    def __init__(self,
                 scene,
                 x_display=None,
        ...
        super().__init__(scene=scene, quality=quality)
    ...
    def reset(self, house,
        ...
        '''
        if not silent:
            print("Resetting ProcThorEnv")

        super().reset(scene=house)
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False):
        ...
        if "RotateLeft" in action:
            ...
            action = dict(action="MoveAhead",
                          forceAction=True, standing=True)
            event = self.step(action, smooth_nav=smooth_nav)
